...
const getRawTransactions = txids => {
  const genesisCoinbaseTransactionId = coins[config.coin].genesisCoinbaseTransactionId;
  ...
  return new Promise((resolve, reject) => {
    ...
    txids.forEach(async txid => {
      if (txid) {
        if (genesisCoinbaseTransactionId && txid == genesisCoinbaseTransactionId) {
          try {
            let blockchainInfoResult = await getBlockchainInfo();
            let result = genesisCoinbaseTransaction;
    ...
    executeBatchesSequentially(utils.splitArrayIntoChunks(requests, 100), batchResult => {
...
function executeBatchesSequentiallyInternal(
  batchId,
  batches,
  currentIndex,
  accumulatedResults,
  ...
  console.log(
    "Executing item #" + (currentIndex + 1) + " (of " + batches.length + ") for batch " + batchId
  );
  ...
  client.command(batches[currentIndex]).then(function(results) {
    ...
    if (count == 0) {
      executeBatchesSequentiallyInternal(
        batchId,
        batches,
        currentIndex + 1,
        accumulatedResults,
